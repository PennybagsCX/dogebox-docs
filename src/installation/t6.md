...
- Plug in a keyboard.
- Plug power in.
- Press the power button once — plugging power in only lights the standby
  LEDs; the box will not boot until you press the button. (Holding the
  button for several seconds force-cuts power — avoid doing that while
  the box is writing to disk.)
...
- Plug power in.
- Press the power button once — plugging power in only lights the standby
  LEDs; the box will not boot until you press the button.
- Wait until the box finishes booting, this could take anywhere from 2 to 10 minutes.
